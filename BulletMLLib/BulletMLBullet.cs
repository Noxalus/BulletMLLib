...
namespace BulletMLLib
{
    ...
    {
        private IBulletMLBulletInterface _ibullet;
        //private float dir;
        public float speedX; //<accel>で使用。移動処理でspeedと加算する必要がある
        public float speedY; //<accel>で使用。移動処理でspeedと加算する必要がある
        //public BulletMLTask task;
        public readonly List<BulletMLTask> Tasks;
        private readonly List<FireData> _fireData;
        ...
        public bool Run()
        {
            ...
            {
                activeTaskNum = i;
                BulletMLTask.BLRunStatus result = Tasks[i].Run(this);
                if (result == BulletMLTask.BLRunStatus.End)
                    endNum++;
            }

            X = X + speedX + (float)(Math.Sin(_ibullet.Dir) * Speed);
            Y = Y + speedY + (float)(-Math.Cos(_ibullet.Dir) * Speed);
        ...
        {
            return _fireData[activeTaskNum];
        }
        ...
        public float X { get { return _ibullet.X; } set { _ibullet.X = value; } }
        public float Y { get { return _ibullet.Y; } set { _ibullet.Y = value; } }
        public float Speed { get { return _ibullet.Speed; } set { _ibullet.Speed = value; } }
        public void Vanish() { _ibullet.Vanish(); }
#if ExpandedBulletML
